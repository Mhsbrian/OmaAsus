...
| **Profiles** | Create, duplicate, rename, recolour, delete, set default. A profile holds a power mode, CPU, GPU, cooling, lighting and a CoolerControl Mode, and can carry firmware limits set in `config.toml`. The first run creates one per power mode the machine has. A power mode set elsewhere (a keyboard shortcut, `powerprofilesctl`, a bar widget) is followed by the profile that carries it |
